...
public class OutputManager {
  ...
  public String nameBabyCount(int year, String gender, char letter) {
    ...
    for (YearOfBirthFile file : reader.getData()) {
      if (file.getMyYear() == year) {
        result += "Total " + year + " " + gender + " names starting with '" + letter + "': " + file
            .namesLetter(gender, letter).size();
        result += "\n";
        result +=
            "Total " + year + " " + gender + " babies whose name Starts with '" + letter + "': "
                + file.totalBabies(gender, letter);
        result += "\n";
      }
    }
